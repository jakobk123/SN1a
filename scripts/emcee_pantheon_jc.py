# -*- coding: utf-8 -*-
"""
Created on Mon Jun 27 16:39:34 2022
...
from cosmo_jnp import pantheon
from cosmo_jnp import cosmo_jc as cosmo
...
def log_prior_planck(Omega_m, w_0=-1, w_1=0):
    '''returns the log of the propability densities up to a constant'''
    if 0.0 < (Omega_m) < 1:
        l_prior = - ((Omega_m - 0.3166)**2/0.0084**2 + (w_0+0.957)**2/0.08**2 + (w_1 + 0.32)**2/0.29**2) 
    else:
        l_prior = -np.inf
    return l_prior

def log_no_prior(Omega_m=0.3, w_0=-1, w_1=0):
    '''returns the log of the propability densities up to a constant'''
    if 0.0 < Omega_m < 1:
        return 0
    return -np.inf

def log_large_planck(Omega_m=0.3, w_0=-1, w_1=0):
    '''returns the log of the propability densities up to a constant'''
    l_prior = - ((Omega_m - 0.3166)**2/(3*0.0084)**2 + (w_0+0.957)**2/(3*0.08)**2 + (w_1 + 0.32)**2/(3*0.29)**2) 
    return l_prior
